...
- **Instancia única**: `main.py` crea un named mutex de Windows (`EcoNodeX_SingleInstance`) al arrancar. Si ya existe (otra instancia en ejecución), el proceso abre el navegador en `localhost:8765` y termina. El mutex se libera automáticamente al cerrar el proceso.
- **Iconos de la aplicación**: el directorio `assets/` contiene `icon.png` (tray, 64×64) e `icon.ico` (exe, multi-resolución). Ambos se generan con `assets/gen_icon.py` y reproducen el diseño de `frontend/public/favicon.svg`. El spec de PyInstaller incluye `assets/` como dato y referencia `icon.ico` para el exe.
